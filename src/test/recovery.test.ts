...
test("recovery final reports locate restored outputs while the shareable version omits paths", () => {
  ...
  const safety: SourceSafety = {
    ...
    sourceDeviceIdentities: [{ path: "/dev/source-physical-test", serial: "source-serial", wwn: null, filesystemUuid: null }],
    sourceTopDevice: "/dev/source-physical-test",
    destinationDevices: ["/dev/destination-physical-test"],
    destinationDeviceIdentities: [{ path: "/dev/destination-physical-test", serial: "destination-serial", wwn: null, filesystemUuid: null }],
