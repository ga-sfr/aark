...
export function buildRecoveryPlan(config: RecoveryConfig): RecoveryPlan {
  ...
  if (config.image.enabled) {
    ...
    steps.push(step({
      ...
      executable: "ddrescue",
      args: ["--idirect", `--retry-passes=${config.image.retryPasses}`, config.source, config.image.path, config.image.mapfile],
      createsDirectories: [],
      outputs: [config.image.path, config.image.mapfile],
      provenance: "allocated-reference",
      destinationWritesExpected: true,
  ...
  if (config.stages.ntfsUndelete) {
    steps.push(step({
      ...
      args: ["--scan", "--verbose", "--parent", analysisSource],
    ...
    steps.push(step({
      ...
      executable: "ntfsundelete",
      args: ["--undelete", "--match", "*", "--destination", path.join(root, "recovery", "ntfsundelete", "files"), analysisSource],
