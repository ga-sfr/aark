...
Block-device checkpoints record stable serial, WWN, and filesystem UUID identifiers from the full disk/partition dependency chain when the kernel exposes them, in addition to device nodes. This retains a partition UUID fallback on USB bridges that expose neither a disk serial nor WWN. A ddrescue quota resume rechecks the prior source and destination identities—including the backing disk of a regular-file image—and refuses an older checkpoint that cannot bind both physical disks stably. Process and operation-lock identities include PID, process group, kernel start ticks, executable device/inode, command fingerprint, and boot identity; PID reuse cannot silently turn a dead workflow lock into a live-owner match, and `EPERM` probing is treated as an existing but unauthorized process rather than a dead or identity-verified one.
