...
The result contains no local paths or recovered values. It reports aggregate scan, finding, marker-only, artifact, retained whole-source file/byte, deletion entry/file/byte counts; whether the recovery and evidence copies are selected; what remains; and a SHA-256 approval token binding the verified state, retained recovery/mining control and report hashes, whole-source retention set, and exact deletion choice. A marker-only finding has no exact exported artifact, but its complete containing source file is retained. Review marker-only files before cleanup if surrounding directory context is still needed.
