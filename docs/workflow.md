...
`aark mine batch` inventories the supplied closed roots, groups at most 128 roots per child scan, and uses configured file and byte ceilings. After completed children it reduces later file ceilings when observed finding or occurrence density approaches 80% of the per-scan inventory bounds. A single root that is itself too dense is blocked with an instruction to subdivide that root; findings already written are not relabeled as a successful scan.

Each child keeps the ordinary frozen manifest, exact artifact tree, reports, and clean-pause resume contract. The batch controller uses a durable active-child checkpoint, recognizes a child that completed just before a controller crash, and commits its aggregate idempotently. A signal between children publishes an explicit resumable pause. Advancement re-verifies the controller lock after the child exits, and aggregate counters are maintained incrementally rather than rescanning every prior root on each progress callback. `batch-aggregate-sensitive.json` holds only category/fingerprint deduplication keys and stays private; `batch-manifest-redacted.json` reports global category and occurrence counts without hashes, values, paths, or offsets.
