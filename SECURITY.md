...
`final-report-sensitive.md`, `cleanup-final-report-sensitive.md`, `inventory-sensitive.json`, `scan-state-sensitive.json`, `scan-files-sensitive.ndjson`, tool logs, raw contexts, and every file under `artifacts/` or `retained-sensitive-source-files/` should be handled as credentials. Artifact files contain exact unredacted values, while retained source files may contain additional sensitive context. The sensitive final report locates those files and source occurrences without needlessly duplicating every value. The CLI attempts mode `0600` for generated sensitive files and `0700` for generated directories. FAT and exFAT do not enforce those permissions; use encrypted storage or physically secure the destination.
...
`aark cleanup plan` performs no deletion. It accepts only a completed recovery case and error-free mining outputs, requires retained recovery controls and reports to match their completed per-run copies, verifies the state/inventory cross-checkpoint, hashes every retained artifact and retained control/report, re-enumerates each frozen mining input, and proves that the union of those inputs covers every regular file in the selected `recovery/` and optional `evidence/` trees. It identifies every distinct source file referenced by a finding and excludes those complete files and their logical bytes from the deletion totals. Its approval token binds the case identity, recovery run, retained control/report hashes, scan checkpoints, every selected directory/file/link/special entry and its metadata, the whole-source retention set, aggregate counts, and evidence choice.

`aark cleanup run` acquires both current and legacy recovery/mining locks, repeats all verification, and rejects a stale token before writing a deletion report. It selects only the case's fixed `recovery/`, `logs/`, and `runs/` children, plus `evidence/` when separately approved. Each target is first atomically moved to an unpredictable case-local quarantine and fully reverified there, preventing a raced path replacement from being recursively deleted. Before recursive removal, every approved finding-containing source file is moved intact, without copying or overwriting, into the private case-local `retained-sensitive-source-files/<APPROVAL_TOKEN>/` tree with its relative path preserved. The approval token binds that retained-source directory's existing identity or absence, and lexical plus device/inode ancestry checks ensure mining output directories overlap neither a deletion target nor the reserved retained-source namespace. Case-root final reports/manifests and complete mining outputs—including `artifacts/`, the sensitive inventory, frozen scan manifest, and scan state—are also retained.

Deletion is irreversible and cannot guarantee secure erasure from SSDs, copy-on-write filesystems, snapshots, backups, or storage-controller caches. A crash or filesystem error can leave a partly populated retained-source tree or a partially removed `.aark-cleanup-pending-*` tree; the cleanup report is written as `authorized-in-progress` first and updated to `failed-partial`, `interrupted-partial`, or `complete` when possible. AARK refuses another cleanup while that quarantine exists. Do not remove it, remove a stale operation lock, or retry a partial cleanup until the local state and mounted filesystems have been inspected.
